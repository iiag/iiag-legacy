...
JNIEXPORT void JNICALL Java_net_lastprime_Main_start(JNIEnv * env, jobject obj, jobjectArray argv)
{
	int i;
	int count = (*env)->GetArrayLength(env, argv);
	...
	for (i = 0; i < count; i++) {
		jtemp = (jstring) (*env)->GetObjectArrayElement(env, argv, i);
		stemp = (char*) (*env)->GetStringUTFChars(env, jtemp, 0);
		
		args[i] = malloc((strlen(stemp)+1) * sizeof(char));
		strcpy(args[i], stemp);
		(*env)->ReleaseStringUTFChars(env, jtemp, NULL);
	}
