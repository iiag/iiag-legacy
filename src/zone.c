...
typedef struct
    { //specifies room with x and y coordinates adn w and h for width and height
	int x, y, w, h;
} room;
...
void zone_draw_tile(zone * z, int x, int y) {
	if (z->tiles[x][y].show == 1 || config.show_all || (z->tiles[x][y].show && z->tiles[x][y].impassible)) {
...
int zone_can_see(zone * z, int x0, int y0, int x1, int y1, int dist) {
	...
	if (sqrt(dx * dx + dy * dy) > (float)dist)
		return 0;
	...
	if (dx > dy) {
		...
		for (x = x0, y = y0; x != x1; x += mx) {
			if (x != x0 && z->tiles[x][y].impassible)
				return 0;
			err += derr;
		...
		for (x = x0, y = y0; y != y1; y += my) {
			if (y != y0 && z->tiles[x][y].impassible)
				return 0;
			err += derr;
